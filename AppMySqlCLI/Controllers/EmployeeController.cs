...
public class EmployeeController
    {
        ...
        {
            var id = _view.GetIdForEdit();
            var existing = _dbContext.GetEmployeeById(id);
            
            if (existing == null)
            {
                _view.ShowErrorMessage($"Karyawan dengan ID '{id}' tidak ditemukan.");
                _view.WaitForKeyPress();
                return;
            }
            ...
            if (_dbContext.UpdateEmployee(existing.Id, updated))
            {
                _view.ShowSuccessMessage("Data karyawan berhasil diperbarui!");
            }
            else
            {
                _view.ShowErrorMessage("Gagal memperbarui data karyawan. ID baru mungkin sudah digunakan oleh karyawan lain.");
            }
        ...
        private void DeleteEmployee()
        {
            ...
            {
                if (_dbContext.DeleteEmployee(id))
                {
        ...
        private void ExportToCsv()
        {
            ...
            try
            {
                ...
                {
                    sb.AppendLine($"{emp.Id};{emp.Name};{emp.Salary.ToString(CultureInfo.InvariantCulture)};{(emp.Status ? "1" : "0")}");
                }
