...
public class EmployeeView
    {
        ...
        {
            return AnsiConsole.Ask<string>("[grey]Masukkan ID Karyawan yang akan diedit:[/]");
        }

        public Employee GetUpdatedEmployeeData(Employee existing)
        {
            AnsiConsole.MarkupLine("[cyan]Edit data karyawan (tekan Enter untuk melewati field):[/]\n");
            AnsiConsole.MarkupLine($"[grey]Data saat ini:[/]");
            AnsiConsole.MarkupLine($"  ID: {existing.Id}");
            AnsiConsole.MarkupLine($"  Nama: {existing.Name}");
            AnsiConsole.MarkupLine($"  Gaji: {existing.Salary:N2}");
            AnsiConsole.MarkupLine($"  Status: {existing.StatusDisplay}\n");

            Console.WriteLine("ID Karyawan baru (Enter untuk tidak mengubah):");
            var idInput = Console.ReadLine();
            var id = string.IsNullOrWhiteSpace(idInput) ? existing.Id : idInput.Trim();
            ...
            while (id.Length > 6)
            {
                ...
                id = string.IsNullOrWhiteSpace(idInput) ? existing.Id : idInput.Trim();
            }

            Console.WriteLine("Nama baru (Enter untuk tidak mengubah):");
            var nameInput = Console.ReadLine();
            var name = string.IsNullOrWhiteSpace(nameInput) ? existing.Name : nameInput.Trim();

            Console.WriteLine("Gaji baru (Enter untuk tidak mengubah):");
            ...
            Console.WriteLine("Status baru (yes/no, Enter untuk tidak mengubah):");
            var statusInput = Console.ReadLine();
            var status = existing.Status;
            if (!string.IsNullOrWhiteSpace(statusInput))
            {
                var statusLower = statusInput.Trim().ToLower();
                if (statusLower == "yes" || statusLower == "1")
                    status = true;
                else if (statusLower == "no" || statusLower == "0")
                    status = false;
            }
        ...
        {
            return AnsiConsole.Ask<string>("[grey]Masukkan ID Karyawan yang akan dihapus:[/]");
        }
        ...
        public bool ConfirmDelete(Employee employee)
        {
            ...
            AnsiConsole.MarkupLine($"Gaji: {employee.Salary:N2}");
            AnsiConsole.MarkupLine($"Status: {employee.StatusDisplay}");
