...
public class DatabaseContext
{
    ...
    public Employee? GetEmployeeByRowId(int rowId)
    {
        using var connection = new MySqlConnection(_connectionString);
        connection.Open();

        var query = "SELECT row_id, id, name, salary, status FROM employees WHERE row_id = @rowId";
        using var command = new MySqlCommand(query, connection);
    ...
    public bool UpdateEmployee(Employee employee)
    {
        try
        {
            ...
            var query =
                "UPDATE employees SET id = @id, name = @name, salary = @salary, status = @status WHERE row_id = @rowId";
            using var command = new MySqlCommand(query, connection);
    ...
    public bool DeleteEmployee(int rowId)
    {
        using var connection = new MySqlConnection(_connectionString);
        connection.Open();

        var query = "DELETE FROM employees WHERE row_id = @rowId";
        using var command = new MySqlCommand(query, connection);
        command.Parameters.AddWithValue("@rowId", rowId);
